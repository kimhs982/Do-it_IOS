...
class ImageViewController: UIViewController {
    ...
    @IBAction func btnResizeImage(_ sender: UIButton) {
        ...
        if (isZoom) {       // true 현재 확대된 그림일 경우(즉, 타이틀은 축소)
            newWidth = imgView.frame.width / scale      // 이미지 뷰의 프레임 너빗값을 scale 값으로 나눔
            newHeight = imgView.frame.height / scale        // 이미지 뷰의 프레임 높잇값을 scale 값으로 나눔
            btnResize.setTitle("확대", for: .normal)      // 버튼의 타이틀을 "확대"로 변경합니다.
        }
        else {      // false
            newWidth = imgView.frame.width * scale      // 이미지 뷰의 프레임 너빗값을 scale 값으로 곱함
            newHeight = imgView.frame.height * scale        // 이미지 뷰의 프레임 높잇값을 scale 값으로 곱함
